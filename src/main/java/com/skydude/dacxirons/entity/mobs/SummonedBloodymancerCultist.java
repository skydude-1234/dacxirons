...
public class SummonedBloodymancerCultist extends BloodymancerCultistEntity implements MagicSummon, GeoAnimatable {

    private static final EntityDataAccessor<Boolean> DATA_IS_ANIMATING_RISE = SynchedEntityData.defineId(SummonedBloodymancerCultist.class, EntityDataSerializers.BOOLEAN);
    protected LivingEntity cachedSummoner;
    protected UUID summonerUUID;
    private int riseAnimTime = 80;

    public SummonedBloodymancerCultist(EntityType<SummonedBloodymancerCultist> type, Level world) {
        super((EntityType) type, world);
        xpReward = 0;

    }
    // Create attributes, inherit original
    @SubscribeEvent
    public static void onEntityAttributeCreation(EntityAttributeCreationEvent event) {
        // start from mcreator builder
        AttributeSupplier.Builder builder = BloodymancerCultistEntity.createAttributes();

        // add the custom attribute that the mcreator  procedure expects
      //  builder.add(DungeonsAndCombatModAttributes.ACTIONSTATE.get(), 0.0D);


        // Add to summoned
        event.put(EntityRegistry.SUMMONED_BLOODYMANCER_CULTIST.get(), builder.build());

    }
    public SummonedBloodymancerCultist(LivingEntity owner, boolean playRiseAnimation) {
        this(EntityRegistry.SUMMONED_BLOODYMANCER_CULTIST.get(), owner.level());
        setSummoner(owner);
    ...
    public void registerGoals() {
        ...
        this.goalSelector.addGoal(1, new RangedAttackGoal(this, 1.25F, 50, 10.0F) {
